...
def draw_keypoint(keypoint, scale):
    ...
    A = np.array([[a, b], [b, c]])
    w, v = np.linalg.eig(A)
    width = 1/np.sqrt(w[0])*2 * scale
    height = 1/np.sqrt(w[1])*2 * scale
    angle = np.arctan2(v[1, 0], v[1, 1])/np.pi*180
    ellipse = matplotlib.patches.Ellipse((x, y), width=width, height=height,
